...
namespace CySoft.Utility.UniqueID
{
    ...
    {
        private static object lock_Generate_19 = new object();
        ...
        private static long datacenterId_Bit = 5L;														// 数据中心ID位数
        private static long machineId_Bit = 5L; 														// 机器ID位数
        private static long sequence_Bit = 12L;                                                         // 序列号位数
        ...
        //public Generate_19(long machineId)
        //{
        ...
        public static long Generate()
        {
            lock (lock_Generate_19)
            {
                ...
                {
                    // LogHelper.Error("Generate_19时间戳比上一次生成ID时时间戳还小");
                    throw new Exception("Generate_19时间戳比上一次生成ID时时间戳还小，故异常");
                }
                Generate_19.lastTimestamp = timestamp; //把当前时间戳保存为最后生成ID的时间戳
                long Id =
                    ((timestamp - twepoch) << (int)timestamp_LShift)
